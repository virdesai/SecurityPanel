...
class SortableGrid extends Component {
    render = () =>
        ...
        >
            {
                this.state.gridLayout && this.items.map((item, key) =>
                    <Block
                        key = { key }
        ...
        if (this.state.activeBlock !== null && this._blockPositionsSet()) {
            ...
            this.state.blockPositions.forEach( (block, index) => {
                if (index !== this.state.activeBlock && block.origin) {
                    ...
                    if (distance < closestDistance && (distance < this.state.blockWidth || distance < this.state.blockHeight)) {
                        closest = index;
            ...
            if (closest !== this.state.activeBlock) {
                ...
                let tempOrderIndex = this.itemOrder[this.state.activeBlock].order;
    ...
    assessGridSize = ({nativeEvent}) => {
        this.blockWidth = nativeEvent.layout.width ? Math.floor(nativeEvent.layout.width / this.itemsPerRow) : this.blockWidth;
        this.blockHeight = nativeEvent.layout.height ? Math.floor(nativeEvent.layout.height / Math.ceil(this.items.length/this.itemsPerRow)) : this.blockHeight;
        if (this.state.gridLayout !== nativeEvent.layout && this.blockHeight > 0 && this.blockWidth > 0) {
            this.setState({
    ...
    reAssessGridRows = () => {
        ...
        if (this.state.blockHeight && oldRows !== this.rows) {
    ...
    setGhostPositions = () => {
        ...
        for (let i = 0; i < ghostBlockCount; i+=1) {
    ...
    deleteBlocks = (deleteBlockIndices) => {
        ...
        this.setState({ blockPositions, blockPositionsSetCount }, () => {
            this.items.forEach((item, order) => {
                let blockIndex = _.findIndex(this.itemOrder, indexItem => indexItem.order === order);
    ...
    _getDeletionView = (key) => this.state.deleteModeOn ? <TabIcon style={ this._getImageDeleteIconStyle(key) } name={'delete-sweep'} /> : null
